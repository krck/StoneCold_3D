...
void SimulationManager::CreateIntroState() {
    try {
        ...
        auto intro = std::make_shared<IntroState>(5, _engine);
...
void SimulationManager::CreateGameState() {
    try {
        ...
        std::string cubePath = "models\\grassCube\\";
#else
        ...
        //	auto cubeModel = _resourceManager->LoadResource<ModelResource>(ResourceLifeTime::Game, (cubePath + "grassCube.obj"));
        auto mapTexture = _resourceManager->LoadResource<TextureResource>(ResourceLifeTime::Game, "mapTextures.png");
        ...
        auto lightColor = glm::vec3(SC_COLOR_YELLOW.x / 255.f, SC_COLOR_YELLOW.y / 255.f, SC_COLOR_YELLOW.z / 255.f);
        auto mapSize = glm::ivec2(256.0f, 256.0f);
        auto mapPreset = PerlinPreset();
        mapPreset.Octaves = 4;              // !NO NEED TO CHANGE! Layers of Noise-Maps. Layers add more detail (rough and more "pronounced" features)
        mapPreset.HeightMulti = 64.0f;      // !NO NEED TO CHANGE! Overall Map-Hight multiplier. Big hills get bigger, low valleys stay the same.
        mapPreset.AmplitudeDampener = 0.5;  // Low: Bigger hills and more "pronounced" terrain - High: Low hills and overall flat terrain
        mapPreset.FrequencyMulti = 2.0f;    // Low: Few, flat and far apart hills - High: Many, big and close to each other hills
        mapPreset.FlattenFactor = 3.5f;     // Low: Produces maps with big hills and deep valles - High: Produces flat maps with minimal elevation
        
        // Generate the Map (Perlin Noise map with a custom calculated and chunked mesh)
        auto chunkMap = _mapManager->GenerateMap(mapSize, mapPreset, lightColor);
        auto mapTransformation = glm::mat4(1.0f);
        mapTransformation = glm::translate(mapTransformation, glm::vec3(1.0f));
        mapTransformation = glm::scale(mapTransformation, glm::vec3(2.0f));
        for (uint32 i = 0; i < chunkMap.size(); i++) {
            // Add the Chunk mesh as a actual MapResource to cleanup based on ResourceLifeTime
            auto chunkName = "map_chunk_" + std::to_string(i + 1);
            auto tmpRes = MapResource(chunkName, chunkMap[i].ChunkVertecies, chunkMap[i].ChunkIndices);
            auto chunk = _resourceManager->AddResource<MapResource>(ResourceLifeTime::Level, chunkName, std::move(tmpRes));
            // Add the Chunk mesh data and its components to the ECS
            auto chunkId = gameECS->CreateEntity();
            gameECS->AddAdditionalSystemMask(chunkId, MASK_SHADER_MAP);
            gameECS->AddComponent<MeshComponent>(chunkId, { chunk->GetVAO(), chunk->GetEBO(), chunk->GetSize() });
            gameECS->AddComponent<TransformationComponent>(chunkId, { mapTransformation });
            gameECS->AddComponent<TextureComponent>(chunkId, { mapTexture->GetTextureId(), 0, 0 });
        }
        ...
        for (MeshResource charMesh : charModel->Model) {
            ...
            charTransformation = glm::translate(charTransformation, glm::vec3(100.0f, 0.0f, 0.0f));
            charTransformation = glm::scale(charTransformation, glm::vec3(5.0f));
            ...
            if(charMesh.Texture != nullptr)
                gameECS->AddComponent<TextureComponent>(charE, { charMesh.Texture->GetTextureId(), charMesh.Texture->GetNormalMapId(), charMesh.Texture->GetSpecularMapId() });
        }
...
void SimulationManager::CreateMenuState() {
    try {
        ...
        auto menu = std::make_shared<MenuState>(10, _engine);
